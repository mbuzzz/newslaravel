(function ($) {
    ...
    const stored = localStorage.getItem(themeKey) || (document.cookie.match(/theme=(dark|light)/)?.[1] ?? 'light');

    applyTheme(stored);

    if (switcher) {
        switcher.checked = stored === 'dark';
    ...
    // Sync header/footer/logo for dark mode dynamically
