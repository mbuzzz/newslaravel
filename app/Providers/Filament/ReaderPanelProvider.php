...
class ReaderPanelProvider extends PanelProvider
{
    public function panel(Panel $panel): Panel
    {
        return $panel
            ->id('reader')
            ->path('reader') // Akses di domain.com/reader
            ->login() // Menggunakan login page default (bisa dishare dengan admin)
            ->profile(EditProfile::class) // Pakai edit profile yang sama dengan admin
            ->colors([
                'primary' => Color::Emerald, // Warna beda (Hijau) biar kerasa bedanya sama Admin
            ])
            ->brandName('Member Area')
